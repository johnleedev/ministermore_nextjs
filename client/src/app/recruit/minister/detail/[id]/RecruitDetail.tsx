...
export default function RecruitDetail ({ postData, id }: { postData: any, id: string }) {


  const router = useRouter();
  const copy = postData;

  const [part, setPart] = useState<any>(copy.part ? JSON.parse(copy.part) : [{sort: '전임', content: ''}]);
  const [partDetail, setPartDetail] = useState<any>(copy.partDetail ? JSON.parse(copy.partDetail) : [{sort: '전임', content: ''}]);
  const [school, setSchool] = useState<any>(copy.school ? JSON.parse(copy.school) : [{sort: '전임', content: ''}]);
  const [career, setCareer] = useState<any>(copy.career ? JSON.parse(copy.career) : [{sort: '전임', content: ''}]);
  const [workday, setWorkday] = useState<any>(copy.workday ? JSON.parse(copy.workday) : [{sort: '전임', content: ''}]);
  const [workTimeSunDay, setWorkTimeSunDay] = useState<any>(copy.workTimeSunDay ? JSON.parse(copy.workTimeSunDay) : [{sort: '전임', startHour: '09', startMinute: '00', endHour: '16', endMinute: '00'}]);
  const [workTimeWeek, setWorkTimeWeek] = useState<any>(copy.workTimeWeek ? JSON.parse(copy.workTimeWeek) : [{sort: '전임', startHour: '09', startMinute: '00', endHour: '17', endMinute: '00', day: '평일'}]);
  const [dawnPray, setDawnPray] = useState<any>(copy.dawnPray ? JSON.parse(copy.dawnPray) : [{sort: '전임', content: ''}]);
  const [pay, setPay] = useState<any>(copy.pay ? JSON.parse(copy.pay) : [{sort: '전임', paySort: 'select', selectCost: '', inputCost: ''}]);
  const [insurance, setInsurance] = useState<any>(copy.insurance ? JSON.parse(copy.insurance) : [{sort: '전임', content: ''}]);
  const [severance, setSeverance] = useState<any>(copy.severance ? JSON.parse(copy.severance) : [{sort: '전임', content: ''}]);
  const [welfare, setWelfare] = useState<any>(copy.welfare ? JSON.parse(copy.welfare) : [{sort: '전임', content: ''}]);
  const [applytime, setApplytime] = useState<any>(copy.applytime ? JSON.parse(copy.applytime) : {startDay: '', endDay: '', daySort: ''});
  const [applydoc, setApplydoc] = useState<any>(copy.applydoc ? JSON.parse(copy.applydoc) : [{sort: '전임', content: ''}]);
  const [inquiry, setInquiry] = useState<any>(copy.inquiry ? JSON.parse(copy.inquiry) : {inquiryName:"", email:"", phone:""});
  const [customInput, setCustomInput] = useState(copy.customInput || '');
