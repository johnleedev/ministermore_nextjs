...
export default function RecruitChurchList ({ recruitList, totalCount, currentPageProp }: { recruitList: any[], totalCount: number, currentPageProp: number }) {
  ...
  const changePage = (newPage: number) => {
    ...
    router.push(`/recruit/church?page=${newPage}`);
  };
  ...
  const handleSearch = async () => {
    ...
    const res = await axios.post(`${MainURL}/api/recruitchurch/recruitsearchlist`, {
      sort: selectedSort,
      location: selectedLocation,
      religiousbody: [],
    })
  ...
  const handleUnifiedSearch = async () => {
    ...
    const res = await axios.post(`${MainURL}/api/recruitchurch/recruitsearchunified`, {
  ...
  const handleItemClick = (itemId: string) => {
    ...
    router.push(`/recruit/church/detail/${itemId}`);
    window.scrollTo(0, 0);
  };
        ...
        <div className="subpage__main">
          ...
          <div className="subpage__main__title">
            ...
            <div className='postBtn'
              onClick={()=>{
                if (isLogin) {
                  router.push('/recruit/church/post');
                } else {
                  alert('로그인이 필요합니다.');
                }
              }}
