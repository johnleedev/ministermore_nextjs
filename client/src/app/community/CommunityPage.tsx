...
export default function CommunityPage({ noticeList, totalCount, currentPageProp }: { noticeList: ListProps[], totalCount: number, currentPageProp: number }) {
  
  const router = useRouter();
  const isLogin = useRecoilValue(recoilLoginState);
  
  const [currentPage, setCurrentPage] = useState<number>(currentPageProp);
  const [list, setList] = useState<ListProps[]>(noticeList);
  ...
  const openPostDetails = async (post: any) => {
    axios.post(`${MainURL}/api/noticeboard/noticepostsviews`, {
      postId: post.id,
      sort : 'notice'
    })
    .then(()=>{
