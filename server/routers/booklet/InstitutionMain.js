...
router.get('/getdatabooklets', async (req, res) => {
  ...
  const query = `SELECT * FROM institutionMain;`;
...
router.post('/getdatabookletspart', async (req, res) => {
  var { id } = req.body;
  const query = `
    SELECT * FROM institutionMain WHERE bookletId = '${id}';
  `;
  bookletdb.query(query, function (error, result) {
    if (error) {
      throw error;
...
router.post('/getdatagallerypart', async (req, res) => {
  var { id } = req.body;
  const query = `
    SELECT * FROM institutionGallery WHERE bookletId = '${id}';
  `;
  bookletdb.query(query, function (error, result) {
    if (error) {
      throw error;
...
// 특정 프로그램 데이터 보내기
router.post('/getdatapracticepart', async (req, res) => {
  var { id } = req.body;
  const query = `
    SELECT * FROM institutionPractice WHERE bookletId = '${id}';
  `;
  bookletdb.query(query, function (error, result) {
    if (error) {
      throw error;
